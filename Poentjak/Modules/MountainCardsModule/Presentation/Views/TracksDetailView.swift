...
struct TracksDetailView: View {
    ...
    @State private var isShowingSelectTrackModal = false
    @State private var selectedDetent = PresentationDetent.fraction(0.5)

    @State private var isShowingPopup = true // Control popup visibility

    var body: some View {
        ZStack {
            VStack {
                TracksMapView(
                    region: $navigateViewModel.region,
                    waypoints: navigateViewModel.gpxParser.parsedFirstLastWaypoints,
                    track: navigateViewModel.gpxParser.parsedTrack,
